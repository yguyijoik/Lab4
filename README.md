...
print(fS-блок Вхідні дані = {bin(data)}, Зашифровані = {bin(encrypted_s_block)}, Розшифровані = {bin(decrypted_s_block)})

permuted_p_block = p_block_permute(data)
reversed_p_block = p_block_reverse_permute(permuted_p_block)
print(fP-блок Вхідні дані = {bin(data)}, Переставлені = {bin(permuted_p_block)}, Зворотня перестановка = {bin(reversed_p_block)})
